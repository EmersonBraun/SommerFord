  ...
  <q-page class="doc-container q-pa-md">
    <q-card flat>
      <q-card-section class="row">
        ...
        <q-btn color="primary" icon="fa fa-plus" class="full-width" label="create" :to="`/fields/create`"/>
      </q-card-section>
    </q-card>
  </q-page>
</template>
...
export default defineComponent({
  ...
  setup (_, { root }) {
    const vars = reactive({
      data: computed(() => root.$store.state.field.data),
      title,
      columns,
      filter: ''
    })
    const functions = {
      async loadData () {
        await root.$store.dispatch('field/getAll')
      },
      editRow (id: number) {
        void root.$router.push(`fields/edit/${id}`)
      },
      removeRow (id: number) {
