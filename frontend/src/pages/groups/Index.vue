  ...
  <q-page class="doc-container q-pa-md">
    <q-card flat>
      <q-card-section class="row">
        ...
        <q-btn color="primary" icon="fa fa-plus" class="full-width" label="create" :to="`/groups/create`"/>
      </q-card-section>
    </q-card>
  </q-page>
</template>
...
export default defineComponent({
  ...
  setup (_, { root }) {
    const vars = reactive({
      data: computed(() => root.$store.state.group.data),
      title,
      columns,
      filter: ''
    })
    const functions = {
      async loadData () {
        await root.$store.dispatch('group/getAll')
      },
      editRow (id: number) {
        void root.$router.push(`groups/edit/${id}`)
      },
      removeRow (id: number) {
        root.$q.dialog(confirmMsg('Alert!','are you sure?')).onOk(() => {
          void root.$store.dispatch('group/remove', id)
          void this.loadData()
        })
      },
    }
