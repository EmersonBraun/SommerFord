...
import ManiesRepository from '../../Repositories/ManiesRepository'
import { ManySchema } from '../../Validators/ManySchema'

export default class ManiesController {
  private readonly repository
  constructor () {
    this.repository = ManiesRepository
  }
  ...
  async moduleList ({ params, response }: HttpContextContract) {
    const register = await this.repository.moduleList(params.id)
    const { data, statusCode, returnType, message, contentError } = register
    return response
      .safeHeader('returnType', returnType)
      .safeHeader('message', message)
  ...
  async store ({ request, response }: HttpContextContract) {
    try {
      await request.validate({schema: ManySchema})
    } catch (error) {
